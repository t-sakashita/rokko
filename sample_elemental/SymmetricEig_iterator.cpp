...
template <class T>
class MyDistMatrix : public elem::DistMatrix<T>
{
  MyDistMatrix(int n, int n);
  std::cout << "ColShift = " << this.ColShift() << std::endl;
};
...
public:
  ...
  CTestIterator* operator=();
  
private:
  CTest* m_obj;      // 走査対象の物体へのポインタ
  int    m_current;  // 現在、指し示している位置
};
