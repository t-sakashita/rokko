...
int main(int argc, char** argv) {
  ...
  if (grid.get_myrank() == 0) {
    ...
    if ((grid.get_nprocs() != grid.get_nprow() * grid.get_npcol()) ||
      ...
      std::invalid_argument("incompatible matrix size and number of processes");
    }
  }

  rokko::mapping_bc<rokko::matrix_col_major> map(n, 1);
  rokko::distributed_matrix<float> a(map);
  rokko::distributed_matrix<float> z(map);
  Eigen::VectorXf w(n);
  for (int j = 0; j < n; ++j)
    for (int i = 0; i < n; ++i)
  ...
  int m, nz;
  int info = rokko::scalapack::psyevr('U', a, m, nz, w, z);
  if (info) {
    std::invalid_argument("Error in rokko::scalapack::psyevr");
  }
  if (grid.get_myrank() == 0)
