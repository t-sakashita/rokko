...
int main(int argc, char** argv) {
  ...
  int il, iu;
  int m, nz;
  int *ifail = alloc_ivector(n);
  int *iclustr = alloc_ivector(2*nprow*npcol);
  double *gap = alloc_dvector(nprow*npcol);
  double orfac = -1;
  double abstol = 0.;

  info = cscalapack_pdsyevx('V', 'A', 'U', n,
                            mat_ptr(a), 0, 0, desc,
                            vl, vu, il, iu,
                            abstol, &m, &nz,
                            vec_ptr(w), orfac,
                            ...
                            ifail, iclustr, gap);
  if (info) {
    fprintf(stderr, "Error in cscalapack_pdsyevx\n");
    MPI_Abort(MPI_COMM_WORLD, info);
  }
  if (myrank == 0) {
    printf("eigenvalues: ");
