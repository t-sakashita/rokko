...
int main(int argc, char *argv[]) {
  ...
  const auto dim = 1 << L;
  const rokko::skel::mapping_1d map{dim, rokko::mpi_comm{MPI_COMM_WORLD}};
  std::vector<double> buffer(map.get_num_local_rows());
  const auto multiply = [&, L, lattice, buffer](const double *const x, double *const y) {
  ...
  const rokko::distributed_mfree_holder mat(multiply, map);
  if (rank == 0)
