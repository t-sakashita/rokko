...
int main(int argc, char *argv[]) {
  ...
  rokko::distributed_matrix<float, matrix_major> mat(map);
  rokko::minij_matrix::generate(mat);
  Eigen::MatrixXf mat_loc(dim, dim);
  rokko::gather(mat, mat_loc, 0);

  Eigen::VectorXf eigval(dim);
  rokko::distributed_matrix<float, matrix_major> eigvec(map);
  rokko::parameters params;
  params.set("routine", routine);
  solver.diagonalize(mat, eigval, eigvec, params);

  Eigen::MatrixXf eigvec_loc(dim, dim);
