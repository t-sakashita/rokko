...
class mfree_f : public rokko::distributed_mfree {
public:
  mfree_f(void (*multiply)(int, const double*, double*), int dim, int num_local_rows)
...
void rokko_distributed_mfree_f_construct(struct rokko_distributed_mfree* matrix,
  ...
  matrix->ptr = new mfree_f(multiply, dim, num_local_rows);
}

void rokko_distributed_mfree_f_destruct(rokko_distributed_mfree* matrix) {
  delete static_cast<mfree_f*>(matrix->ptr);
}
