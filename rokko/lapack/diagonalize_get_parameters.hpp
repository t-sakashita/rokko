...
inline char get_matrix_part(parameters const& params) {
...
inline std::string get_matrix_part(char const& uplow) {
  if (uplow == 'U')
    return "upper";
  else if (uplow == 'L')
    return "lower";
...
inline void print_verbose(std::string const& routine, char const& jobz, char const& uplow) {
...
template<typename T>
inline void print_verbose(std::string const& routine, char const& jobz, char const& range, char const& uplow,
...
inline bool is_interval(parameters const& params) {
