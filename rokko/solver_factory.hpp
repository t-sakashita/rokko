...
#define ROKKO_REGISTER_SOLVER(solver, name) \
  namespace { namespace BOOST_JOIN(solver_register, __LINE__) { struct register_caller { register_caller() { rokko::solver_factory::instance()->register_creator<solver>(name); } } caller; } }
