...
void diagonalize(rokko::distributed_matrix<MATRIX_MAJOR>& mat, double* eigvals,
                 rokko::distributed_matrix<MATRIX_MAJOR>& eigvecs, timer& timer_in) {
  if(mat.is_row_major())
    throw "eigen_sx doesn't support matrix_row_major.  Use eigen_sx with matrix_col_major.";
  ROKKO_eigen_init(mat.get_grid().get_comm(), (mat.get_grid().is_row_major() ? 'R' : 'C'));
  int dim = mat.get_m_global();
  int lld = mat.get_lld();
  timer_in.start(1);
  ROKKO_eigen_sx(dim, dim, mat.get_array_pointer(), lld, eigvals, eigvecs.get_array_pointer(),
                 lld, 8, 128);
  timer_in.stop(1);
  ROKKO_eigen_free(1);
}
