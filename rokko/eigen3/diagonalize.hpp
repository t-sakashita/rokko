...
parameters diagonalize(Eigen::Matrix<T,ROWS,COLS,MATRIX_MAJOR>& mat, Eigen::Vector<real_t<T>, SIZE>& eigvals,
...
parameters diagonalize(Eigen::Matrix<T,ROWS,COLS,MATRIX_MAJOR>& mat, std::vector<real_t<T>>& eigvals_in,
		       rokko::parameters const& params) {
  parameters params_out;
  if (get_matrix_part(params) == 'U') {
    throw std::invalid_argument("eigen3::diagonalize() : Eigen3's SelfAdjointEigenSolver does not support upper part of matrix.");
