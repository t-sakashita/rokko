...
#ifndef ROKKO_UTILITY_MINIJ_MATRIX_HPP
#define ROKKO_UTILITY_MINIJ_MATRIX_HPP
...
public:
  ...
  static void generate(rokko::localized_matrix<T, MATRIX_MAJOR>& mat) {
    if (mat.rows() != mat.cols())
      BOOST_THROW_EXCEPTION(std::invalid_argument("minij_matrix::generate() : non-square matrix"));
    int n = mat.rows();
    for(int i = 0; i < n; ++i) {
      for(int j = 0; j < n; ++j) {
...
#endif // ROKKO_UTILITY_MINIJ_MATRIX_HPP
