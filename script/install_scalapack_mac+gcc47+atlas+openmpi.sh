...
cmake -DCMAKE_INSTALL_PREFIX=$PREFIX -DCMAKE_C_COMPILER=openmpicc -DCMAKE_Fortran_COMPILER=openmpif90 -DLAPACK_LIBRARIES="-L/opt/local/lib -llapack -lptcblas -lptf77blas -latlas -lgfortran" -DBUILD_SHARED_LIBS=ON -DBUILD_STATIC_LIBS=OFF -DCMAKE_INSTALL_NAME_DIR=$PREFIX/lib $HOME/build/scalapack-2.0.2
