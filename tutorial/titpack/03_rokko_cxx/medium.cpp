...
int lnc2(crs_matrix const& mat, int nvec, int iv, std::vector<double>& E,
  ...
  if (wk.rows() < 2 || wk.cols() != mat.dimension()) wk.resize(2, mat.dimension());
...
void lncv2z(crs_matrix const& mat, int nvec, int iv,
  ...
  if (x.rows() < nvec || x.cols() != mat.dimension()) x.resize(nvec, mat.dimension());
...
double check2(crs_matrix const& mat, const double *x, matrix_type& v, int vindex) {
  if (v.rows() < vindex || v.cols() != mat.dimension()) v.resize(vindex, mat.dimension());
