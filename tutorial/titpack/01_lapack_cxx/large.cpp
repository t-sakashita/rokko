...
double check1(int n, std::vector<int> const& ipair, std::vector<double> const& bondwt,
  ...
  double dnorm = 0;
  for (int i = 0; i < idim; ++i) dnorm += x(xindex, i) * x(xindex, i);
  if (dnorm < 1e-30) {
  ...
  for (int k = 0; k < ibond; ++k) {
    ...
    for (int i = 0; i < idim; ++i) {
      int ibit = list1[i] & is;
      if (ibit == 0 || ibit == is) {
        v(vindex, i) -= diag * x(xindex, i);
      } else {
        v(vindex, i) += diag * x(xindex, i);
        int iexchg = list1[i] ^ is;
        int ia = iexchg & irght;
        int ib = (iexchg & ilft) / ihfbit;
        v(vindex, i) -= x(xindex, list2[0][ia] + list2[1][ib]) * wght;
      }
    }
  }
  
  double prd = 0;
  for (int i = 0; i < idim; ++i) prd += v(vindex, i) * x(xindex, i);
  ...
  for (int i = std::min((int)(idim / 3), 13) - 1; i < idim; i += std::max(1,idim/20), ++count) {
    ...
    std::cout << '\t' << v(vindex, i) / x(xindex, i);
  }
  std::cout << std::endl
            << "---------------------------------------------------\n";
  return prd;
