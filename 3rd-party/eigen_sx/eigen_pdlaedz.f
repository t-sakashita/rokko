      SUBROUTINE EIGEN_PDLAEDZ(COMPZ, HBW, N, N1, ID, Q, IQ, JQ, LDQ, 
     $                       DESCQ, Z, WORK, V )
...
*  =======
*
*  EIGEN_PDLAEDZ Form the z-vector which consists of the last row of Q_1
...
*
     ...
     $              IQROW, IQCOL )
      ...
      IF (COMPZ == 'F') GOTO 50
*
*     Z = Q'*V,  Q = DirectSum(Q1, Q2)
*

*
*     Form z1 which consist of the last two rows of Q1
*
      DO K = 1, HBW
...
*
     ...
     $                        IZ1ROW, COL )
                 ...
                 DO 10 J = 1, NBLOC
                    ZSIZ = MIN( NB, NQ1-IZ1 )
                    CALL DCOPY( ZSIZ, WORK( IBUF+IZ1 ), 1, 
     $                          TEMP( IZ ), 1 )
                    IZ1 = IZ1 + NB
                    IZ = IZ + NB*NPCOL
10               CONTINUE
...
20         CONTINUE
*
*          ADD THE NEW RECEIVED VECTOR TO Z
*
           IF (K .EQ. 1) THEN
                CALL DCOPY(N1, TEMP, 1, Z, 1)
            ELSE
                CALL DAXPY(N1, 1.0D+0, TEMP, 1, Z, 1)
            ENDIF
        END IF
      END DO
*
*     Form z2 which consist of the first 2 rows of Q2
*
...
*
     ...
     $                            IZ2ROW, COL )
                 ...
                 DO 30 J = 1, NBLOC
                    ZSIZ = MIN( NB, NQ2-IZ2 )
                    CALL DCOPY( ZSIZ, WORK( IBUF+IZ2 ), 1, 
     $                          TEMP( IZ ), 1 )
                    IZ2 = IZ2 + NB
                    IZ = IZ + NB*NPCOL
   30            CONTINUE
...
*
           IF (K .EQ. 1) THEN
                CALL DAXPY(N2, 0.0D+0, Z(N1+1), 1, TEMP(N1+1), 1)
                CALL DCOPY(N2, TEMP(N1+1), 1, Z(N1+1), 1)
            ELSE
                CALL DAXPY(N2, 1.0D+0, TEMP(N1+1), 1, Z(N1+1), 1)
            ENDIF
...
*
*     Q'*V, Q is a dense matrix
*     Need to sum up the 4 rows: IQ-1+(ID+N1-2)  ... IQ-1+(ID+N1+1)
*     that usually not on the same node
*
     ...
     $           NPCOL, MYROW, MYCOL, IIZ2, JJZ2, IZ2ROW, IZ2COL )
          ...
          IF( ( MYROW.EQ.IZ2ROW ) .AND. ( NQ1.NE.0 ) ) THEN
             CALL DCOPY( NQ1, Q( IIZ2, JJZ2 ), LDQ, WORK, 1 )
             DO J = 1, NQ1
                 WORK (J) = V(K) * WORK (J)
             END DO
...
*
          IF( MYROW.EQ.IQROW .AND. MYCOL.EQ.IQCOL ) THEN
             ...
             COL = IZ2COL
             DO 70 I = 0, NPCOL - 1
                NQ1 = NUMROC( N, NB, COL, IZ2COL, NPCOL )
                IF( NQ1.GT.0 ) THEN
                   IF( IZ2ROW.NE.IQROW .OR. COL.NE.IQCOL ) THEN
                      IBUF = N1 + 1
                      CALL DGERV2D( ICTXT, NQ1, 1, WORK( IBUF ), NQ1,
     $                          IZ2ROW, COL )
                   ELSE
...
*
              ...
              IF (K .EQ. 1) THEN
!                 CALL DAXPY(N, 0.0D+0, Z, 1, TEMP, 1)
                  CALL DCOPY(N, TEMP, 1, Z, 1)

              ELSE
                  CALL DAXPY(N, 1.0D+0, TEMP, 1, Z, 1)
              ENDIF
