*	This subroutine is modified from scalapack's EIGEN_PDSxEDC to 
*       calculate eigenvalues and eigenvectors of a pentadiagonal matrix
*       in parallel using divide and conquer

      SUBROUTINE EIGEN_PDSxEDC( COMPZ, HBW, N, D, E, LDE, Q, IQ, JQ,  
     $                   DESCQ, WORK, LWORK,IWORK, LIWORK, INFO )

*     .. Scalar Arguments ..
      CHARACTER          COMPZ
      INTEGER            INFO, IQ, JQ, LIWORK, LWORK, N, HBW
*     ..
*     .. Array Arguments ..
      INTEGER            DESCQ( * ), IWORK( * )
...
*  Purpose
*  =======
*  EIGEN_PDSxEDC computes all eigenvalues and eigenvectors of a
...
*     .. External Subroutines ..
     ...
     $                   INFOG2L, EIGEN_PDLAED0, PDLASRT, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCQ( CTXT_ ), 'EIGEN_PDSxEDC', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
50       CONTINUE
      END IF
*
      CALL EIGEN_PDLAED0( N, HBW, D, E, LDE, Q, IQ, JQ, DESCQ, WORK,
     $                    IWORK, NP, NQ, INFO )
